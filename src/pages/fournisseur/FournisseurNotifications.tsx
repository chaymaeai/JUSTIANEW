...
const TYPE_META: Record<string, { label: string; icon: string; color: string; bg: string }> = {
  nouvelle_demande:       { label: "Nouvelle demande",  icon: "📋", color: "text-cyan-600",    bg: "bg-cyan-50 dark:bg-cyan-950/30" },
...
export default function FournisseurNotifications() {
  ...
  const handleClick = async (n: Notification) => {
    ...
    if (n.demande_id) {
      navigate(`/fournisseur/demandes?id=${n.demande_id}`);
    }
  };
  ...
  const filterTabs: Array<{ id: FilterType; label: string; icon: string }> = [
    ...
    { id: "nouvelle_demande",       label: "Demandes",      icon: "📋" },
    { id: "demande_assignee",       label: "Assignations",  icon: "👤" },
    { id: "document_ajoute",        label: "Documents",     icon: "📎" },
    { id: "consultation_planifiee", label: "Consultations", icon: "📅" },
                ...
                {count > 0 && (
                  <span
                    className={`rounded-full px-1.5 py-0.5 text-[10px] font-bold ${
                      filter === tab.id ? "bg-white/20" : "bg-red-100 text-red-600"
                    }`}
                  >
                    {count}
                  </span>
